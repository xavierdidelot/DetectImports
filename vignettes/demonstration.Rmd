...
```{r}
pvals=test0(dates,coalints,Ne=Ne(0))
summary(pvals)
```
...
If we apply the same test as before, using for example the mean of the $A_i$ as the estimator of $N_e$, we will get some false positives in the detection of imports:

```{r}
pvals=test0(dates,coalints,Ne=mean(coalints))
summary(pvals)
```
...
Let's try this:
```{r}
pvals=test1(dates,coalints,epsilon=1)
summary(pvals)
```
