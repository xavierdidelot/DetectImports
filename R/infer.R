...
#' @param tree Tree
#' @param model One of 'constant' 'gp' 'gpcontam'
#' @param adjust Method for adjusting p-values (default is none)
...
#' @return Results of importation test
#' @export
detectImports=function(tree,model='gp',adjust='none',verbose=T,diagnostics=F,nchains=4,iter=4000,seed=NULL)
{
  if (is.null(tree$stats)) tree=keyStats(tree)
  m=tree$stats
  ...
  {
    mod <- cmdstan_model(system.file('stan','gpmodel_v2.stan',package='DetectImports',mustWork = T),compile=F)
  ...
  } else if (model== 'gpcontam') {
    mod <- cmdstan_model(system.file('stan','gpmodel_v2_contam.stan',package='DetectImports',mustWork = T),compile=F)
    data_list <- list(N = length(coalints), intervals=coalints, T_s=dates, M=20, c=2.0)
    coalnames <- sapply(c(1:length(coalints)),function(i) paste0("coal_means[",i,"]"))
  } else{
    stop(paste("Unrecognised option:",model))
  }

  if (verbose) {
    mod$compile()
    fit <- mod$sample(data = data_list,adapt_delta=0.95, chains = nchains,parallel_chains = nchains,refresh = round(iter*0.1),iter_sampling = round(iter*0.75),iter_warmup = round(iter*0.25),seed=seed)
  } else {
    invisible(capture.output(suppressMessages({
      mod$compile()
      fit <- mod$sample(data = data_list,adapt_delta=0.95, chains = nchains,parallel_chains = nchains,refresh = 0,show_messages=F,iter_sampling = round(iter*0.75),iter_warmup = round(iter*0.25),seed=seed)
    })))
  }
