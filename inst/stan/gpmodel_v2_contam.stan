functions {
   ...
   //Square root of spectral density function associated with Matern kernel with nu=3/2, dimension D=1 and Euclidiean distance
...
parameters {
    ...
    real<lower = 0> alpha_tilde;  // kernel magnitude (ie marginal variance)
    real<lower = 0> l; // kernel length scale
}

transformed parameters {
    real alpha = 0.5*alpha_tilde;
    vector[N] a_coeffs;
    vector[N] coal_means;
    vector[M] spec_dens;
    vector[M] lp;
...
model {
    ...
    for(idx in 1:M) { 
        ...
        v[2] = log(eps) + cauchy_lpdf(intervals[idx] | 0, 1);
        target += log_sum_exp(v);
    }
}
